def generate_ld_r_r(registers, writer):
    ...
    for d_code, d_name in registers.items():
        for s_code, s_name in registers.items():
            ...
            writer.writerow([
                opcode_hex, 1, f"LD {d_name}, {s_name}", "PC", f"REG_{d_name}", f"REG_{s_name}", "PC_INC", 1, 0, 1
            ])
...
def generate_ld_r_n(registers, writer):
    ...
    for code, name in registers.items():
        ...
        writer.writerow([opcode_hex, 2, f"LD {name},n M2", "PC", f"REG_{name}", "REG_MEM", "REG_COPY",
                         "PC_INC", 1, 0, 1])
...
def generate_ld_r_hl(registers, writer):
    ...
    for code, name in registers.items():
        ...
        writer.writerow([
            ...
            "HL", f"REG_{name}", "REG_MEM", "REG_COPY", "NONE", 1, 0, 1
        ])
...
def generate_ld_a_bc(registers, writer):
    ...
    writer.writerow([opcode_hex, 2, "LD A, (BC) M2", "BC", "REG_A", "REG_MEM", "A_FROM_Z", "NONE", 1, 0, 1])
...
def generate_ld_a_de(registers, writer):
    ...
    writer.writerow([opcode_hex, 2, "LD A, (DE) M2", "DE", "REG_A", "REG_MEM", "A_FROM_Z", "NONE", 1, 0, 1])
...
def generate_ld_a_nn(registers, writer):
    ...
    writer.writerow([opcode_hex, 4, "LD A, (nn) M4", "WZ", "REG_A", "REG_MEM", "A_FROM_Z", "NONE", 1, 0, 1])
...
def generate_ldh_a_c(registers, writer):
    ...
    writer.writerow([opcode_hex, 2, "LDH A, (C) M3", "FF00_C", "REG_A", "REG_MEM", "A_FROM_Z", "NONE", 1, 0, 1])
...
def generate_ldh_a_n(registers, writer):
    """
    ...
    writer.writerow([opcode_hex, 3, "LDH A,(n) M3", "FF00_Z", "REG_A", "REG_MEM", "A_FROM_Z", "NONE", 1, 0, 1])

def generate_ldh_n_a(registers, writer):
    """
    Load to the 8-bit A register, data from the address specified by the 8-bit immediate data n. The
    full 16-bit absolute address is obtained by setting the most significant byte to 0xFF and the
    least significant byte to the value of n, so the possible range is 0xFF00-0xFFFF.
